...
uint256 hashGenesisBlock("0x000009f2f92c872f2e59949ce3df1d1202b98fe19081fbc9b95a8fcee4dab055");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DexCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DexCoin Signed Message:\n";
...
{
    // DexCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 24 * 60 *60; // DexCoin: 24 hours
static const int64 nTargetSpacing = 60; // DexCoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DexCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, DexCoin - DarkGravity, written by Evan Duffield - evan@DexCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DexCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // DexCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000f13547ccf9058b6e41a3241d7da3ffbf054c8f62846c6ab2806ce517ff3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "Canadian Regulator Introduces New Rules for Crypto Mining";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1556702325;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 116304;

        if (fTestNet)
        {
            block.nTime    = 1556702090;
            block.nNonce   = 1064716;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x69afc80e9d9f9c123ec1af0ec2a005c018402f1e63a776cee71212696320032a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb4, 0x2c, 0xd5, 0xe1 }; 
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DexCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DexCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DexCoinMiner(CWallet *pwallet)
{
    printf("DexCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("DexCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running DexCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DexCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DexCoinMiner, pwallet));
}
